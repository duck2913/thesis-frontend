					...
					<div className="flex w-max gap-8 mt-2">
						{foods.map((food) => (
							<div className="food relative" key={food.imgUrl}>
								...
								<div>{food.name}</div>
								<div className="font-semibold text-orange-500">{food.price}</div>
								<button className="p-0 absolute bottom-[3.5rem] right-1 bg-green-600 w-8 h-8 rounded-full flex items-center justify-center">
									<AiOutlinePlus />
								</button>
							</div>
						))}
					...
					<div className="flex w-max gap-8 mt-2">
						{drinks.map((drink) => (
							<div className="drink relative" key={drink.imgUrl}>
								...
								/>
								<div>{drink.name}</div>
								<div className="font-semibold text-orange-500">{drink.price}</div>
								<button className="p-0 absolute bottom-[3.5rem] right-1 bg-green-600 w-8 h-8 rounded-full flex items-center justify-center">
									<AiOutlinePlus />
								</button>
							</div>
